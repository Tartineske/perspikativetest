...
// ============================= 14. MOBILE : PANNEAU PERSPECTIVE SOUS HEADER =============================
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let ticking = false;
  const maxScroll = 150;
  const panelOffset = -10; // légèrement sous le header pour masquer le bord haut
  ...
  const ensurePanel = () => {
    ...
    if (!panel) {
      ...
      panel.className = 'mobile-vanish-panel';
  ...
  const resetPanel = () => {
    root.style.setProperty('--mobile-vanish-opacity', '0');
    root.style.setProperty('--mobile-vanish-tilt', '0deg');
    root.style.setProperty('--mobile-vanish-scale-x', '1');
    root.style.setProperty('--mobile-vanish-scale-y', '1');
    root.style.setProperty('--mobile-vanish-shift-y', '0px');
    root.style.setProperty('--mobile-vanish-inset', '0%');
  };
  ...
  const updatePanel = () => {
    ...
    const panelTop = Math.max(0, headerRect.bottom + panelOffset);

    const progress = Math.min(scrollTop / maxScroll, 1);
    const eased = 1 - Math.pow(1 - progress, 2);

    root.style.setProperty('--mobile-vanish-top', `${panelTop}px`);
    root.style.setProperty('--mobile-vanish-opacity', (0.12 + eased * 0.42).toFixed(3));
    root.style.setProperty('--mobile-vanish-tilt', `${(-0.35 - eased * 2.6).toFixed(3)}deg`);
    root.style.setProperty('--mobile-vanish-scale-x', (1 - eased * 0.14).toFixed(4));
    root.style.setProperty('--mobile-vanish-scale-y', (1 - eased * 0.06).toFixed(4));
    root.style.setProperty('--mobile-vanish-shift-y', `${(-eased * 14).toFixed(2)}px`);
    root.style.setProperty('--mobile-vanish-inset', `${(eased * 10).toFixed(3)}%`);
